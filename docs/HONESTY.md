...
## Planned Multigres cover-note update

When the serving and CDC planes are far enough along to discuss the database and coordination layer, the cover note (and the README's CDC section) will be updated to address Multigres directly: where Harbormaster uses managed Postgres plus Debezium today, and where a Vitess/Multigres-style sharded, coordinated Postgres would change the design. This update will state plainly that Harbormaster does not currently implement the sharded query router or the consensus layer that Multigres provides, that those remain the two named out-of-scope items above, and that the comparison is aspirational architecture discussion, not a claim of having built it. The update is tracked as a TODO against the CDC and serving phases and will be grounded in the actual database module once that module exists.
