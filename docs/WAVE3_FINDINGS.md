...
| n/a | n/a | `serving/app/components/space_time_prism.py`, `serving/app/agents/gap_detector.py` | Arithmetic longitude deltas treated a short antimeridian crossing as a path around almost the entire planet, and a single lon/lat Polygon drew a world-spanning closing edge. | Wrapped focus deltas and midpoints now follow the shortest direction. Geometry is cut at each crossed antimeridian seam and emitted as a normalized MultiPolygon with GeoJSON ring winding. Gap distance uses haversine distance, and DRM compares each MOBR component's bounds so a seam cut does not create a false Greenwich overlap. Local regressions cover both crossing directions, exact and noncanonical seams, intersections, dynamic merge, and short-distance gap scoring. Transcript: `docs/drills/ANTIMERIDIAN_PRISM_LOCAL_2026-07-13.md`. |
| n/a | n/a | `serving/app/components/space_time_prism.py` | The local equirectangular plane became ill-conditioned near either geographic pole. A valid 10 m by 0.1 m footprint at latitude 89.99999 raised `polygon spans 360 degrees or more`, while a projected 10 m radial offset measured only 2.172325 m geodesically. | Replaced it with a spherical azimuthal-equidistant projection centered on the active foci's spherical midpoint, switched focus distance to wrapped haversine, and retained continuous inverse longitude for the existing seam cutter. Adaptive sampling and conservative MOBR polygonization keep supported rotated footprints valid and bounding at extreme latitude, while corner-only mapping preserves infeasible zero-axis prisms. Antipodal or ill-conditioned foci, and any requested ellipse or MOBR that contains or touches either pole, fail with deterministic domain errors. Focused local verification passed 75 tests at 90.85% combined line and branch coverage. Transcript: `docs/drills/NEAR_POLE_PRISM_LOCAL_2026-07-13.md`. |
...
The near-pole projection debt is closed locally. The supported domain now
includes near-pole ellipses and MOBRs only when the requested footprint does not
contain or touch either geographic pole. Pole containment or contact and
antipodal or numerically singular foci are rejected deterministically. This is
local geometry evidence only; no AWS command or deployed serving process was
used. See `docs/drills/NEAR_POLE_PRISM_LOCAL_2026-07-13.md`.
