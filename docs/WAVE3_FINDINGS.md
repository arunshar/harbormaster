...
Remaining projection debt: the local equirectangular prism kernel is unstable
near the poles. Antimeridian crossings now use the shortest wrapped longitude
delta and GeoJSON-safe seam cuts. Ordinary output remains a Polygon; a geometry
cut at the antimeridian is externally visible as a MultiPolygon. Finding 20
and the antimeridian hardening do not close the near-pole projection case.
