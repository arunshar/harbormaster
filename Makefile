...
apply-plan: init
	@test -n "$(PLAN)" || (echo "PLAN is required, for example: make apply-plan PLAN=artifacts/w4.tfplan" >&2; exit 2)
...
phase5-render-serving: ## W4: render digest-pinned EKS YAML from the expected ECR repository
